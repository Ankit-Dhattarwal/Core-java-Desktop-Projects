	...
	class CalListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent evt) 
		{
			...
			if(bb==bt[17])
			{
				...
				int i=str.indexOf('.');
				//This condition will be true when str will have .
				if(i!=-1)
					return;
			}
			...
			//Checking digits button and . button
			...
			if(bb==bt[1])
			{
				//Removing current entry of textbox
				tb.setText("0");
			}
			//Checking back button
			if(bb==bt[0])
			{
				//Removing last digit from the textbox
				String str=tb.getText();
				if(str.equals("0"))
					return;
				if(str.length()==1)
